...
mciconf.controller('mainController', ['$scope', '$rootScope', '$http',
  function mciconf($scope, $rootScope, $http) {
    ...
    $scope.addBuild = function () {
      ...
      newEmptyBuild.platform_version =  newEmptyBuild.platform.versions.filter(function(value) {
        return !value.added;
      })[0];
      newEmptyBuild.platform_version.added = true;
      if (!newEmptyBuild.platform.versions.filter(function(value) {
        return !value.added;
      }).length) {
      ...
      $scope.$watch(function () {
        ...
        oldVersion.added = false;
        if (!newVersion.versions.filter(function(value) {
          return !value.added;
        }).length) {
          newVersion.added = true;
        }
  ...
  return {
    ...
    controller: function ($scope, $rootScope, $timeout) {
      ...
      $scope.addVersion = function (aIndex) {
        $rootScope.builds[aIndex].firefox_versions.push({buildsFound: 0});
      };
      ...
      $scope.checkBuild = function (aVersionIndex, aBuildIndex) {
        ...
        if (!$rootScope.builds[aBuildIndex].firefox_versions[aVersionIndex].name &&
          ...
          $rootScope.builds[aBuildIndex].firefox_versions[aVersionIndex].buildsFound = 2;
          return;
        }
          ...
          x.onreadystatechange = function () {
            if (x.readyState === 2 && x.status === 200) {
              ...
              if (foundBuilds === locales.length) {
                $rootScope.builds[aBuildIndex].firefox_versions[aVersionIndex].buildsFound = 1;
      ...
      $scope.checkLocales = function (aVersionIndex, aBuildIndex) {
        $rootScope.builds[aBuildIndex].firefox_versions[aVersionIndex].buildsFound = 0;
      ...
      $scope.updatePlatformVersion = function (aIndex) {
        $rootScope.builds[aIndex].platform_version = $rootScope.builds[aIndex].platform.versions.filter(function(value) {
          return !value.added;
        })[0];
      };
      ...
      $scope.$on('checkAll', function () {
        ...
        for (var i = 0; i < length; i += 1) {
          $scope.checkBuild($scope.$index, i);
        }
      });
    }
  }
    ...
    controller: function ($scope, $rootScope) {
      ...
      $scope.platformsLeft = function () {
        if (!$rootScope.platforms)
          return false;
        return $rootScope.platforms.filter(function (value) {
          return !value.added;
        }).length
      }
    }
